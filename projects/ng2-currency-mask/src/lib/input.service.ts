...
export class InputService {
  ...
  fixCursorPosition(forceToEndPosition?: boolean): void {
    ...
    if (
      currentCursorPosition > this.getRawValueWithoutSuffixEndPosition() ||
      forceToEndPosition
    ) {
      ...
      currentCursorPosition < this.getRawValueWithoutPrefixStartPosition()
    ) {
      this.inputManager.setCursorAt(
        this.getRawValueWithoutPrefixStartPosition()
      );
    }
  }
  ...
  removeNumber(keyCode: number): void {
    ...
    if (selectionEnd == selectionStart) {
      ...
      if (
        ...
        /^\d+$/.test(this.rawValue.substring(selectionStart, selectionEnd + 1))
      ) {
      ...
      if (
        ...
        (this.rawValue.substring(selectionStart, selectionEnd + 1) == decimal ||
          this.rawValue.substring(selectionStart, selectionEnd + 1) ==
            thousands)
      ) {
      ...
      if (
        keyCode == 8 &&
        /^\d+$/.test(this.rawValue.substring(selectionStart - 1, selectionEnd))
      ) {
        selectionStart = selectionStart - 1;
      }
      ...
      if (
        ...
        (this.rawValue.substring(selectionStart - 1, selectionEnd) == decimal ||
          this.rawValue.substring(selectionStart - 1, selectionEnd) ==
            thousands)
      ) {
